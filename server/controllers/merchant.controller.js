...
const signupController = async (req, res, next) => {
	try {
		...
		const info = await sendEmail({
			...
			html: `<h1>Dear Admin <br> Kindly review the onboarding information for the merchant - ${user.email}</h1>`,
			text: `Dear Admin, 
        Kindly review the onboarding information for the merchant - ${user.email}
        `,
		});
		...
		res
			.status(201)
			.send(
				responseCreator("Merchant Sign Up Request Sent Successfully", QRcode)
			);
		// console.log(QRcode)
		// res.send(
		//   `<h1>
...
const loginController = async (req, res, next) => {
	try {
		...
		res.status(200).send(responseCreator("Merchant Logged In Successfully!", data));
	} catch (error) {
		next(error);
	}
};
const loginWithTokenController = async (req, res, next) => {
	try {
		...
		res
			.status(200)
			.send(responseCreator("Merchant Logged In With Token Successfully!", user));
	} catch (error) {
		next(error);
	}
};
const logoutController = async (req, res, next) => {
	try {
		...
		res.send(responseCreator("Merchant Logged Out Successfully"));
	} catch (error) {
		next(error);
	}
};
